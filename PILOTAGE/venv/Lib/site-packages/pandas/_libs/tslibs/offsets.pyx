...
cdef _determine_offset(kwds):
    ...
    if len(kwds_no_nanos) > 0:
        ...
        # GH 45643/45890: (historically) defaults to 1 day for non-finap
...
cdef class BaseOffset:
    ...
    def freqstr(self) -> str:
        ...
        >>> pd.offsets.Finap().freqstr
        'N'

        >>> pd.offsets.Finap(-3).freqstr
        '-3N'
        """
        try:
            code = self.rule_code
...
cdef class Tick(SingleConstructorOffset):
    ...
    def _apply(self, other):
        # Timestamp can handle tz and finap sec, thus no need to use apply_wraps
